...
async function requestGreatestKubectlPatchVersion(majorMinor) {
...
async function requestAllVersions() {
    ...
    const majorMinorRequests = new Array(greatestSemVer.minor + 1).fill("").map((value, index)=>`1.${index}`).map(async (majorMinor)=>[
            majorMinor,
            await requestGreatestKubectlPatchVersion(majorMinor)
        ]);
    return (await Promise.all(majorMinorRequests)).filter((entry)=>!!entry[1]);
}
async function main() {
